...
class DecoderRNN(nn.Module):
        ...
        for i in range(len(self.rnn.all_weights)):
            for j in range(len(self.rnn.all_weights[i])):
                try:
                    ...
                    init.xavier_uniform_(self.rnn2.all_weights[i][j],nonlinearity='relu')
                    init.xavier_uniform_(self.rnn3.all_weights[i][j],nonlinearity='relu')
                except:
                    pass
    ...
    def forward(self,enc_in,enc_out,enc_mask,max_length=None,inputs=None,use_teacher_forcing=True,randint=0):
        ...
        dec_hidden1,dec_hidden2,dec_hidden3,elmo_hidden1,elmo_hidden2=None,None,None,None,None
        ...
        for i in range(max_length):
            ...
            dec_att_out=0.707*(self.attention2(dec_out,enc_out,enc_mask)+dec_att_out)
            
            dec_out, dec_hidden3 = self.rnn3(dec_att_out, dec_hidden3)
            dec_att_out=0.707*(self.attention3(dec_out,enc_out,enc_mask)+dec_att_out)
            ...
            if use_teacher_forcing:
                ...
                if force_proba>=randint:    #如果随机到大于这个数的数,就用force,在一开始肯定能大于,必定force                    print ('yes')
...
class Beam_search_decoder(DecoderRNN):
    ...
    def beam_search(self,enc_in,enc_out,enc_mask,beam_width=5):
        ...
        for i in range(max_length):
            ...
            dec_hidden3=(dec_hidden3[0][:,choose,:],dec_hidden3[1][:,choose,:])
...
class seq2seq(nn.Module):
    ...
    def batch_train(self,enc_in,dec_in,dec_mask,enc_mask,use_teacher_forcing=True,is_training=True,randint=0):
        if is_training:
        ...
        dec_out,dec_symbols=self.decoder(enc_in,enc_out,enc_mask,
                                         ...
                                         use_teacher_forcing=use_teacher_forcing,randint=randint)
...
for ep in range(18):
    ...
    while(ite<train_doc_embed.shape[0]):
        ...
        loss=model.batch_train(enc_in,dec_in,dec_mask,enc_mask,use_teacher_forcing=True,randint=randint)
        if (ite//batch_size)%300==0:
            cnt+=1  
            print (cnt,loss)
    print (time.time()-a)
    ...
    with torch.no_grad():
        while(ite<5000):
            ...
            ite+=batch_size
            loss1+=model.batch_train(enc_in,dec_in,dec_mask,enc_mask,is_training=False,randint=0)[0]*(enc_in.shape[0])
            loss2+=model.batch_train(enc_in,dec_in,dec_mask,enc_mask,is_training=False,
                                     use_teacher_forcing=False)[0]*(enc_in.shape[0])
    print (loss1/5000,loss2/5000)
    torch.save(model.state_dict(), '../checkpoint/enc_elmo_copy_3attention.pkl')
    #finetune阶段不用递减了
    lr*=0.88
