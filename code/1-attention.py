...
class Beam_search_decoder(DecoderRNN):
    '''TODO beam search还可以修改。应该改成像veterbi算法那样。当前结点选beamwidth个不一样的进入下一步才对，如果当前结点是一样的
        那后面的无论如何都不如第一高，后续的搜索没有意义，应该改为不同节点。
    '''
    def __init__(self,arg):
    ...
    def beam_search(self,enc_in,enc_out,enc_mask,beam_width=5):
        ...
        for i in range(max_length):
            '''第一步，将上次的attenton输出和这次的input拼接起来'''
            '''在beamsearch中,由于很多都很上一时刻有关,因此这个顺序必须必须要认真调换!!!!!'''
            in_embed,elmo_hidden1,elmo_hidden2=self.dec_elmo_embed(dec_symbol,elmo_hidden1,elmo_hidden2)
            ...
            if i==0:
                ...
                '''different beamsearch'''
                '''两种beamsearch各有好处.我认为,应该结合两种的优点,一开始尽可能选取不同的token,
                后面才允许选择相同的token,设置阈值为8'''
                '''这两种方法都试一试好了..'''
                if i<=20:
...
class seq2seq(nn.Module):
    ...
    def batch_train(self,enc_in,dec_in,dec_mask,enc_mask,use_teacher_forcing=True,is_training=True):
        if is_training:
    ...
    def batch_predict(self,enc_in,enc_mask):
        enc_out=self.encoder(enc_in)
        dec_out,dec_symbols=self.decoder(enc_in,enc_out,enc_mask,use_teacher_forcing=False)
        return dec_symbols
...
for ep in range(18):
    ...
    while(ite<train_doc_embed.shape[0]):
        ...
        loss=model.batch_train(enc_in,dec_in,dec_mask,enc_mask,use_teacher_forcing=True)
    ...
    with torch.no_grad():
        while(ite<5000):
            ...
            ite+=batch_size
            loss1+=model.batch_train(enc_in,dec_in,dec_mask,enc_mask,is_training=False)[0]*(enc_in.shape[0])
    ...
    torch.save(model.state_dict(), '../checkpoint/enc_elmo_copy.pkl')
    lr*=0.88
