...
def beam_search(enc_in,enc_mask,TFIN,beam_width=5):
    ...
    for i in range(max_length):
        ...
        a_proba=F.softmax(a_dec_to_vocab,dim=2).squeeze()
        b_proba=F.softmax(b_dec_to_vocab,dim=2).squeeze()
        c_proba=F.softmax(c_dec_to_vocab,dim=2).squeeze()
        d_proba=F.softmax(d_dec_to_vocab,dim=2).squeeze()
        e_proba=F.softmax(e_dec_to_vocab,dim=2).squeeze()
        f_proba=F.softmax(f_dec_to_vocab,dim=2).squeeze()
        g_proba=F.softmax(g_dec_to_vocab,dim=2).squeeze()
        h_proba=F.softmax(h_dec_to_vocab,dim=2).squeeze()
        
        proba=torch.log((a_proba+b_proba+c_proba+d_proba+e_proba+f_proba+g_proba+h_proba)/8+1e-12)+beam_proba
...
for i in tqdm(range(test_doc_embed2.shape[0])):
    ...
    f= open('../result/%d.txt'%(fi),'w',encoding='utf-8')
    f.write(test_pred3[i])
    f.close()
        
for i in tqdm(range(test_doc_embed2.shape[0])):
    fi=int(idx[i])
    f= open('../team_result/myresult1/%d.txt'%(fi),'w',encoding='utf-8')
    f.write(test_pred3[i])
    f.close()
